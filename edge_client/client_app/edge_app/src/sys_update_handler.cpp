...
bool SysUpdateHandler::isNeedDownload(const FileInfo_t &t)
{
    ...
    {
        if (f.filename == t.filename && f.save_path == t.save_path && f.filesize == t.filesize && boost::iequals(f.md5, t.md5))
        {
...
void SysUpdateHandler::loadLocalFileInfos()
{
    ...
    b = readFile2Str(fileInfosPath, fileInfos);
    if (!b)
    {
        AMS_ERR("read {} error!", fileInfosPath);
        return;
    }
    g_messagebus.sendMessage(kMsgFileInfos, fileInfos);

    b = loadFileInfosFromStr(fileInfos, m_fileInfo_vec);
    if (!b)
    {
        AMS_ERR("loadFileInfosFromStr {} error!", fileInfos);
        return;
    }
    b = checkFileInfos(m_fileInfo_vec);
    if (!b)
    {
        AMS_ERR("checkFileInfos {} error!");
        return;
    }
...
void SysUpdateHandler::updateFileInfos(const FileInfo_t &t)
{
    ...
    const std::string fileInfosPath = boost::filesystem::current_path().string() + std::string("/file_infos.json");
    std::string fileInfos = writeFileInfos2Str(m_fileInfo_vec);
    writeFileInfos2File(fileInfosPath, fileInfos);
