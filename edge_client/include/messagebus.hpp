...
public:
    // regist message
    template< class... _Args, class _Func, class = typename std::enable_if<!std::is_member_function_pointer<_Func>::value>::type>
    void attach(uint32_t key, _Func && func)
    {
    ...
    // non-const member function
    template<class... _Args, class _Class, class... _DeclareArgs, class _Object>
    void attach(uint32_t key, void(_Class::*func)(_DeclareArgs...), _Object & object)
    {
    ...
    // const member function
    template<class... _Args, class _Class, class... _DeclareArgs, class _Object>
    void attach(uint32_t key, void(_Class::*func)(_DeclareArgs...) const, _Object & object)
    {
        std::function<void(_Args...)> fn = [&, func](_Args... args) { return (object.*func)(std::forward<_Args>(args)...); };
        m_map.insert(std::make_pair(key, fn));
    }
    ...
    // Broadcast messages, call all the callback functions
    template<typename... _Args>
    void sendMessage(uint32_t key, _Args... args)
    {
        auto range = m_map.equal_range(key);
        for (auto it = range.first; it != range.second; it++)
        {
...
public:
    ...
    ~MessageBus2() = default;
private:
    MessageBus2(const MessageBus2&) = delete;
    MessageBus2& operator=(const MessageBus2&) = delete;

    std::multimap<uint32_t, boost::any> m_map;
};
