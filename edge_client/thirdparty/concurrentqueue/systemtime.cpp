...
double getTimeDelta2(SystemTime start)
{
    timespec t;
    CompilerMemBar();
...
double getTimeDelta(SystemTime& start)
{
    ...
    auto diff = static_cast<double>(static_cast<long>(t.tv_sec) - static_cast<long>(start.tv_sec)) * 1000 + double(t.tv_nsec - start.tv_nsec) / 1000000;
    start = t;
    return diff;
}
